 ...
 * @extends {Component}
 */
export default class CollectHome extends Component {
	constructor(props) {
		super(props);
	...
	//组件初始化完毕
	componentDidMount() {
		//订阅事件，用于刷新页面（搜索钢企名录页面删除后，返回需要刷新页面）
			...
			<View style={{ flex: 1 }}>
				...
				>
					<View style={{ flex: 1, flexDirection: 'row' }}>
						<Text style={{ fontSize: 15, color: 'black' }}>{++index}</Text>
						<Text
							style={{
								paddingLeft: 10,
								flex: 8,
					...
					<View style={{ flex: 1, flexDirection: 'row' }}>
						...
						>
							<View style={{ flex: 1, flexDirection: 'row' }}>
								<Text style={{ flex: 1, textAlign: 'right' }}>主营产品:</Text>
								<Text style={{ flex: 3, paddingLeft: 10 }} numberOfLines={1}>
									{item.prod}
								</Text>
							</View>
							<View
							...
							>
								<Text style={{ flex: 1, textAlign: 'right' }}>钢厂:</Text>
								<Text style={{ flex: 3, paddingLeft: 10 }} numberOfLines={1}>
									{item.steel}
								</Text>
							</View>
							<View style={{ flex: 1, flexDirection: 'row' }}>
								<Text style={{ flex: 1, textAlign: 'right' }}>联系人:</Text>
								<View style={{ flex: 3, flexDirection: 'row' }}>
									...
									<Text style={{ paddingLeft: 10 }}>
										{item.mobile.indexOf('/') == -1 ? item.mobile : item.mobile.split('/')[0]}
									</Text>
								</View>
							</View>
						</View>
						...
						>
							<TouchableWithoutFeedback onPress={() => this._callPhone(item)}>
								<Image style={{ width: 40, height: 40 }} source={image.tool.tell_y} />
							</TouchableWithoutFeedback>
						</View>
					</View>
				</View>
	 ...
	 */
	searchaData = () => {
		this.nav.navigate('steelCollectSearch');
	};
	...
	render() {
		return (
			<View>
				...
				<FlatList
					style={{
						height: Platform.OS == 'ios' ? height - 160 : height - 180
					}}
						...
						{this.state.photos.map((item, index) => {
							return (
								...
								>
									...
									>
										<Image style={{ width: 25, height: 25 }} source={image.tool.tell_y} />
										<Text style={{ paddingLeft: 12 }}>{item}</Text>
									</View>
								</TouchableHighlight>
							);
						})}
